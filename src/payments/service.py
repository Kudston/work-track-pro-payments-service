...
class PaymentService():
    def __init__(self, client, requesting_user, db, app_settings) -> None:
        ...
        self.crud = PaymentCrud(
            client=client, db=db, urls=UrlsConfiguration.parse_obj(app_settings.urls),
            )
    def get_task_wages(
        self,
        task_id: UUID
    ...
    def get_all_tasks_wages_total(
        ...
        try:
            ...
            #         BaseForbiddenException("You are not allowed to perform this operation.")
            #         )
            wages_total = self.crud.calculate_all_task_wage(start_date=start_date, end_date=end_date, unpaid=unpaid)
            data = {
                "total_amount":wages_total,
            }
            return success_service_result(schemas.TotalTaskWageAmount.parse_obj(data))
    ...
    def compute_all_task_wages(
        ...
        start_date: date,
        end_date: date,
    )->ServiceResult[Union[dict, Exception]]:
        try:
            # if not self.requesting_user.Role == "ADMIN":
            #     return failed_service_result(
            #         BaseForbiddenException("You are not allowed to perform this operation.")
            #         )
            self.crud.compute_all_tasks_wages(start_date, end_date)
            return success_service_result({"detail":"Computation underway."})
        except Exception as raised_exception:
            return failed_service_result(raised_exception)
    ...
    def compute_task_wages(
        ...
        try:
            ...
            self.crud.compute_task_wages(task_id)
            return success_service_result({"detail":"Task wages computation underway."})
        except Exception as raised_exception:
            return failed_service_result(raised_exception)
    ...
    def send_user_payment(
        ...
        try:
            ...
            return success_service_result(
                schemas.ManyWagesOut.parse_obj(wages_paid)
            )
        except Exception as raised_exception:
            return failed_service_result(
                raised_exception
